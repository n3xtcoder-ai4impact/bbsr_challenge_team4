...
class Config(metaclass=Singleton):
    ...
    def __init__(self):
        ...
        try:
            self.is_local = (
                True if os.environ["IS_LOCAL"].upper().strip() == "TRUE" else False
            )
        ...
        try:
            ...
            api_version_file_path = (
                "." + api_version_file_path if self.is_local else api_version_file_path
            )
        ...
        try:
            self.debug = (
                False
                if self.configparser["API"]["DEBUG"].upper().strip() == "FALSE"
                else True
            )
        ...
        try:
            ...
            self.logger.log(
                ...
                500,
                "Could not parse the config: " + str(e),
                "app.configuration.getConfig.Config",
            )

        self.configuration_dict = self.config.get_dict_anon(
            exclude=["pwd", "password", "secret", "url"]
        )
        print(
            "CONFIGURATION: (Some key/value pairs are anonymized or not present due to sensitive data)"
        )
