...
class DatasetUpdater:
    ...
    def _get_available_versions(self)->Dict:
        ...
        response = self._contact_api(params={'format': 'json'})
    ...
    def _is_update_necessary(self)->Tuple[bool, Optional[str]]:
    ...
    def _download_new_version(self, new_version_uuid: str):
        """Downloads a new Oekobaudat dataset version with the passed UUID."""
        response = self._contact_api(url_tail=f'{new_version_uuid}/exportCSV')
        if not response.status_code == 200:
            ...
            self._handle_new_version_response(response)
            self._write_downloaded_version_details_to_json()

        logger.info(f'Update process completed')


    def _handle_new_version_response(self, response):
        ...
        if os.path.exists(output_path):
            ...
            if self._overwrite_smaller_file(new_file=output_path_temp, old_file=output_path):
                logger.success(
    ...
    def _contact_api(self, url_tail: str = '', params=None, headers=None):
    ...
    def update(self):
        ...
        update_necessary, update_uuid = self._is_update_necessary()
        if update_necessary:
            logger.info('Update is possible, will download.')
            self._download_new_version(update_uuid)
        else:
    ...
    def _write_downloaded_version_details_to_json(self):
    ...
    def _overwrite_smaller_file(self, new_file: str, old_file: str):
