...
class DatasetUpdater:
    ...
    def get_available_versions(self) -> Dict:
        ...
        if not response.status_code == 200:
            logger.error(
                "Could not update list of available Oekobaudat data sets. Update aborted."
            )
            available_versions = {}
        else:
            available_versions = {
                item["uuid"]: [item["name"][0]["value"], item["shortName"]]
                for item in response.json()["dataStock"]
            }
            logger.info(
                f"Got {len(available_versions)} available versions from Oekobaudat API."
            )
    ...
    def is_update_necessary(self) -> Tuple[bool, Optional[str]]:
        ...
        if not available_versions:
            ...
            try:
                oekobaudat_versions_old = read_json_file(
                    file_path="app/data/OBD/oekobaudat_versions_old.json"
                )
                self.new_version_uuid = list(
                    set(available_versions.keys()) - set(oekobaudat_versions_old.keys())
                )[0]
            except IndexError:
                pass
            ...
            if not self.new_version_uuid:
                ...
                if "release" in available_versions[self.new_version_uuid][0].lower():
                    ...
                    self.new_version_description = available_versions[
    ...
    def download_new_version(self, new_version_uuid: str):
        ...
        if not response.status_code == 200:
            logger.error(
                f"Update failed, response status_code:{response.status_code}. Could not update Oekobaudat data to release XXX with uuid YYY"
            )
        else:
    ...
    def handle_new_version_response(self, response):
        ...
        if os.path.exists(output_path):
            ...
            if self.overwrite_smaller_file(
                new_file=output_path_temp, old_file=output_path
            ):
                logger.success(
                    f"Updated existing Oekobaudat file {self.new_version_name} with UUID {self.new_version_uuid} to a more recent version"
                )
                self.new_version_message = (
                    "Updated the existing file to a more recent version"
                )

        else:
            write_csv_from_response(response, output_path=output_path)
            logger.success(
                f"Downloaded new Oekobaudat file {self.new_version_name} with UUID {self.new_version_uuid}"
            )
    ...
    def perform_update(self):
        ...
        if update_necessary:
            ...
            self.new_version_message = (
                "No new OBD releases found - no update necessary."
            )
            logger.info(self.new_version_message)

        return UpdateResponse(
            message=self.new_version_message,
            name=self.new_version_name,
            description=self.new_version_description,
            uuid=self.new_version_uuid,
        )
