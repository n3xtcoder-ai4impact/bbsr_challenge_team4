...
def uuid_input_handler(
    uuid_input: str, obd: pd.DataFrame, specific_generic_mapping: pd.DataFrame
) -> UuidsOut:
    ...
    for file in [obd]:
        if uuid_input in file["UUID"].unique():
            ...
            if "generic dataset" in list(file[file["UUID"] == uuid_input]["Typ"]):
                ...
                df_generic_matches = list(
                    specific_generic_mapping[
                        specific_generic_mapping["Specific_UUID"] == uuid_input
                    ]["Generic_UUID"]
                )
                logger.info(f"UUID is a non-generic dataset.")
                if len(df_generic_matches) == 0:
                    message = (
                        "Could not find any generic materials that match the input uuid"
                    )
                    logger.info(f"{message}")
                else:
    ...
    return UuidsOut(uuid_in=uuid_input, uuids_out=uuid_output, message=message)
