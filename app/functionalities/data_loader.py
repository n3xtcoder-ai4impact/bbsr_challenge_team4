...
class DataLoader:
    ...
    def __init__(self):
        ...
        self.tbaustoff = pd.read_csv(
            "app/data/tBaustoff/tBaustoff_with_OBD_mapping.csv",
            encoding="utf-8",
            low_memory=False,
        )
        logger.info(f"Loaded tBaustoff.csv")
        self.specific_generic_mapping = pd.read_csv(
            "app/data/semantic_matching/specific_generic_mapping.csv", low_memory=False
        )
        logger.info(f"Loaded specific_generic_mapping.csv")

        self.current_dataset_version = self.load_dataset_version(
            "app/data/OBD/current_dataset_version.json"
        )
    ...
    def _load_obd_directory(self, directory: str) -> pd.DataFrame:
        ...
        for csv_file in path.glob("*.csv"):
            ...
            try:
                df = pd.read_csv(
                    csv_file, delimiter=";", encoding="latin-1", low_memory=False
                )
