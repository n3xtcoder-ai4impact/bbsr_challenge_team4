...
def read_form(request: Request):
    return templates.TemplateResponse(
        "index.html", context={"request": request, "result": "none"}
    )
...
def form_post(request: Request):
    ...
    return templates.TemplateResponse(
        "input.html", context={"request": request, "result": result}
    )


@router.post("/input")
def form_post(
    request: Request, uuid_input: str = Form(None), update: bool = Form(False)
):
    ...
    if uuid_input is None:
        return templates.TemplateResponse(
            "input.html",
            context={"request": request, "result": "Please enter a valid UUID"},
        )
    if not uuid_input.replace("-", "").isalnum():
        ...
        if not uuids_out:
            return templates.TemplateResponse(
                "input.html",
                context={"request": request, "result": f"{response.message}"},
            )
        else:
            return templates.TemplateResponse(
                "input.html",
                context={
                    ...
                    + "<br>".join(
                        [f"{i + 1}: {uuid}" for i, uuid in enumerate(uuids_out)]
                    ),
                },
            )
...
async def health() -> dict:
    ...
    status_dict = {
        ...
        "description": "Health",
        "details": {
            "Just another check": {"status": "UP", "description": "description"},
        },
    }
