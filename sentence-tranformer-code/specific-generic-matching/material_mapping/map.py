...
def calculate_scores(specific_uuid, specific, generic, generic_name_embeddings, generic_cat_embeddings) -> pd.DataFrame:
    ...
    for i, (_, gen_row) in enumerate(generic.iterrows()):
        ...
        year_score = year_bucket_match(specific.iloc[specific_index[0]]["Referenzjahr"], gen_row["Referenzjahr"])
        unit_score = unit_match(specific.iloc[specific_index[0]]["Bezugseinheit"], gen_row["Bezugseinheit"])
...
def map_materials(specific: pd.DataFrame, generic: pd.DataFrame, generic_name_embeddings: torch.Tensor, generic_cat_embeddings: torch.Tensor, specific_name_embeddings, specific_cat_embeddings) -> pd.DataFrame:
    specific_generic_mapping = pd.DataFrame()
    specific_generic_mapping = pd.DataFrame(columns=["Specific_UUID", "Generic_UUID", "Specific_Name", "Generic_Name", "Final_Score"])
    ...
    specific_generic_mapping.to_csv("data/result/specific_generic_mapping.csv", index=False)
...
if __name__ == "__main__":
    ...
    specific_name_embeddings = model.encode(specific["Name (en)"].fillna("").tolist(), convert_to_tensor=True)
    specific_cat_embeddings = model.encode(specific["Kategorie (en)"].fillna("").tolist(), convert_to_tensor=True)
